...
test('User can create a new List', async (t) => {
  ...
  await t
    .expect(
      Selector('#list-name').withText('First List', { timeout: 2000 }).exists
    )
    .ok();
  await t
...
test('Can mark entries as completed', async (t) => {
  ...
  await t
    ...
    .expect(Selector('h4').withText('First List').exists)
    .ok();
...
test('User can update an already existing list', async (t) => {
  ...
  await t
    .expect(
      Selector('#list-description').withText('Updated List Description').exists
    )
    .ok();
});
