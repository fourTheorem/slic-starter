...
test('Can add entries to newly created list', async t => {
  await t
    ...
    .pressKey('enter')
    .expect(Selector('span').withText('New Item 1')).exists

  await t
    .typeText('#newEntryTitle', 'New Item 2', { replace: true })
    .pressKey('enter')
    .expect(Selector('span').withText('New Item 2')).exists

  await t
    .typeText('#newEntryTitle', 'Another Entry', { replace: true })
    .pressKey('enter')
    .expect(Selector('span').withText('Another Entry')).exists

  await t
    .typeText('#newEntryTitle', 'Last Entry', { replace: true })
    .pressKey('enter')
    .expect(Selector('span').withText('Last Entry')).exists
})
...
test('Can update existing entries', async t => {
  ...
  await t
    .click(Selector('Button').withAttribute('name', 'New Item 1'))
    .expect(Selector('Button').withText('Edit')).exists

  await t
    .click(Selector('#edit-entry'))
    .typeText('#edit-entry', 'Updated Value', { replace: true })
    .click(Selector('#save-btn'))
    .expect(Selector('span').withText('Updated Value')).exists
})
...
test('Can delete an entry from an existing list', async t => {
  ...
  await t
    .click(Selector('Button').withAttribute('name', 'Updated Value'))
    .expect(Selector('Button').withText('Delete')).exists
  await t
    .click(Selector('#delete-entry'))
    .expect(Selector('h6').withText('Delete Entry?')).exists

  const confirmBtn = Selector('#entry-confirmation-confirm-btn')
  await t
    .click(confirmBtn)
    .expect(Selector('span').withText('New Item 1').exists).notOk
})
...
test('User can update an already existing list', async t => {
  ...
  await t
    .typeText(titleTextfield, 'Updated List Title')
    .expect(titleTextfield.value)
    .eql('Updated List Title')
    .click(descriptionTextField)
    .pressKey('ctrl+a delete')
    .typeText(descriptionTextField, 'Updated List Description')
    .expect(descriptionTextField.value)
    .eql('Updated List Description')
    .click(Selector('#save-btn'))
    .expect(Selector('h2').withText('Updated List Title')).exists
...
test('Can remove a full list, including entries', async t => {
  ...
  await t
    .click(deleteListConfirmBtn, { timeout: 1000 })
    .expect(
